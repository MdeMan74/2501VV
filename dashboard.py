...
FILE_BATT = "results_580.csv"      # SAM-export: rij1 runs (1..4), rij2 scaling, rijen 3-5 n.v.t., vanaf rij6 data
FILE_EXISTING_LOAD = "BestaandeLoad.csv"  # Bestaande load (kW per kwartier)
...
def sam_parse_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    Verwacht kolommen: 'run', '1','2','3','4','5'
...
existing_load_df = read_csv_robust(Path(FILE_EXISTING_LOAD))
if cap_df is None or bat_raw is None or existing_load_df is None:
    st.stop()
...
with st.sidebar:
    ...
    # Bestaande load keuze
    st.subheader("Bestaande load")
    existing_load_col = st.selectbox(
        "Kolom in BestaandeLoad.csv",
        options=list(existing_load_df.columns),
        index=0
    )
    include_existing_load = st.checkbox(
        "Voeg bestaande load toe aan som flows",
        value=False,
        help="Wanneer aangevinkt wordt de bestaande load opgeteld bij de som van flows naar load"
    )

    # Jaarladen-keuze -> map naar kolom positie 1..4
    st.subheader("Laden (MWh/jaar) → kolom 1..4")
    chosen_mwh = st.selectbox("Kies jaarladen (MWh)", [100, 200, 300, 400, 500], index=0)
    col_map = {100: "1", 200: "2", 300: "3", 400: "4", 500: "5"}
    ...
    # Tijdsperiode selectie voor gedeelde zoom over alle grafieken
    st.subheader("Gedeelde tijdsperiode grafieken")
    time_preset = st.selectbox(
        "Kies tijdsperiode", 
        ["Eerste week januari", "Eerste week juni", "Eerste maand", "Heel jaar", "Custom"],
        index=1  # Default: Eerste week juni
    )
    
    if time_preset == "Eerste week januari":
        time_range_start = "2025-01-01"
        time_range_end = "2025-01-08"
    elif time_preset == "Eerste week juni":
        time_range_start = "2025-06-01"
        time_range_end = "2025-06-08"
    elif time_preset == "Eerste maand":
        time_range_start = "2025-01-01"
        time_range_end = "2025-02-01"
    elif time_preset == "Heel jaar":
        time_range_start = "2025-01-01"
        time_range_end = "2025-12-31"
    else:  # Custom
        time_range_start = st.date_input("Zoom start", value=pd.Timestamp("2025-06-01").date()).strftime("%Y-%m-%d")
        time_range_end = st.date_input("Zoom eind", value=pd.Timestamp("2025-06-08").date()).strftime("%Y-%m-%d")
...
existing_load_series = pd.to_numeric(existing_load_df[existing_load_col], errors="coerce").reset_index(drop=True)
...
existing_load_kw = existing_load_series  # bestaande load is al kW

# ---- Lengtes gelijk trekken (grid_import_kw is al op kwartierbasis) ----
n = min(len(cap_kw), len(batt_kw), len(grid_kw), len(sys_kw), len(grid_import_kw), len(soc_series), len(existing_load_kw))
cap_kw         = cap_kw.iloc[:n].reset_index(drop=True).fillna(0)
existing_load_kw = existing_load_kw.iloc[:n].reset_index(drop=True).fillna(0)
...
# Som van flows naar load (kW) - bestaande load wordt altijd meegeteld
sum_flows_kw = (batt_kw.fillna(0) + grid_kw.fillna(0) + sys_kw.fillna(0) + existing_load_kw.fillna(0))

# Optionele alternatieve som zonder bestaande load (voor vergelijking)
sum_flows_without_existing = (batt_kw.fillna(0) + grid_kw.fillna(0) + sys_kw.fillna(0))
if not include_existing_load:
    sum_flows_kw = sum_flows_without_existing

# Datetime x-as
start_dt = datetime.combine(start_date, start_time)
t = pd.date_range(start=start_dt, periods=n, freq=f"{step_minutes}min")

# Shared time range for synchronized zooming across all charts
shared_time_range = [time_range_start, time_range_end]
if 'time_range' not in st.session_state:
    st.session_state.time_range = shared_time_range
else:
    # Update session state with new range
    st.session_state.time_range = shared_time_range

# Ruimte op aansluiting (kW) = beschikbaar - grid inkoop
# (bestaande load zit al verwerkt in de onderliggende data)
ruimte_kw = (cap_kw - grid_import_kw).fillna(0)
ruimte_neg = ruimte_kw.where(ruimte_kw < 0, 0)  # alleen negatieve delen voor rood

# Totale belasting op grid (voor visualisatie doeleinden)
total_load_on_grid = grid_import_kw + existing_load_kw
...
# Grid → Load (verborgen maar in stackgroup)
fig1.add_trace(go.Scatter(x=t, y=grid_kw.clip(lower=0),  name="Grid → Load",    mode="lines", stackgroup="one", visible="legendonly"))
# Bestaande load als flow (in stack)
fig1.add_trace(go.Scatter(x=t, y=existing_load_kw.clip(lower=0), name="Bestaande Load → Load", mode="lines", stackgroup="one", line=dict(color="purple")))
# Somlijn (verborgen en niet gestapeld)
som_label = "Som flows → Load" + (" + Bestaande" if include_existing_load else "")
fig1.add_trace(go.Scatter(x=t, y=sum_flows_kw,           name=som_label,
...
fig1.update_layout(
    ...
    xaxis=dict(range=st.session_state.time_range)  # Use synchronized time range
)
# Shared time range across all charts
shared_chart = st.plotly_chart(fig1, use_container_width=True, key="flows_stacked")

# Get the current time range from the chart (if user has zoomed/panned)
# This will be used to sync all other charts
...
existing_energy = existing_load_kw.clip(lower=0).sum() * hours_per_step

# Bestaande load wordt altijd bij grid energy opgeteld (komt van grid)
total_grid_energy = grid_energy + existing_energy
total_energy = batt_energy + total_grid_energy + sys_energy
if total_energy <= 0:
    total_energy = 1e-9

labels = ["Battery", "Grid", "System", "Total Load"]
sources = [0, 1, 2]
targets = [3, 3, 3]
values  = [batt_energy, total_grid_energy, sys_energy]
percents = [v / total_energy * 100 for v in values]

# Specifieke labels voor grid flow (inclusief bestaande load)
grid_detail = f"Grid → Load ({grid_energy*100/total_energy:.1f}%) + Bestaande → Load ({existing_energy*100/total_energy:.1f}%)"
link_labels = [
...
fig_sk.update_layout(
    ...
    # Note: Sankey diagrams don't have time axes, so no synchronization needed
)
...
# Bestaande load lijn
cap_vs_grid.add_trace(go.Scatter(x=t, y=existing_load_kw, mode="lines", name="Bestaande Load (kW)", line=dict(color="purple", width=2)))
# Totale belasting op grid (grid inkoop + bestaande load)
cap_vs_grid.add_trace(go.Scatter(x=t, y=total_load_on_grid, mode="lines", name="Totale belasting grid (kW)", line=dict(color="red", dash="dot")))
...
with st.expander("Debug: eerste 10 waardes"):
    st.write(pd.DataFrame({
        ...
        "existing_load_kw": existing_load_kw[:10].values,
    }))
...
cols = {
    ...
    "existing_load_kw": existing_load_kw,
